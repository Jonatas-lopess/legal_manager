...
Report set (PLANNING §4, unchanged from planning doc — confirm against wireframes once available):
...
- Prazos próximos/vencidos (priority metric per §1 — likely reuses `deadlines` listing's overdue/vence-em-breve comparison already built in `deadlines-engine-alerts`, per that spec's "no separate report query — the full aggregate dashboard is step 6, out of scope here")
...
- **Module boundary/interface contract**: per repo convention (see `deadlines-engine-alerts/spec.md`'s "Modules" bullet), this is decided inside this spec's Implementation Decisions once scope is settled — no standalone contract file. Draft once screens doc confirms exactly which aggregates each screen needs (shapes the `reports.controller.ts` export surface).
